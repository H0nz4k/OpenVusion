...
from .configuration import DEFAULT_CONFIG, deep_merge, load_config, write_example_config
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="hwsniff",
        description="OpenVusion HWSniff — Pi Zero 2 W headless GPIO (default) "
        "or legacy Waveshare touch UI.",
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--gpio-test",
        action="store_true",
        help="Run LED/DIP/button/WLAN hardware self-test and exit",
    )
    parser.add_argument(
        "--mock-gpio",
        action="store_true",
    ...
    # Default: headless Pi Zero GPIO app
