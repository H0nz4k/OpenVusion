...
class SimpleProtocolClient:
    ...
    def iso14443_3_tdx(
        ...
        """Transparentní ISO14443-3 výměna přes TWN4 Simple Protocol.

        `tx` musí obsahovat kompletní RF rámec. TWN4 v tomto režimu
        automaticky nedoplňuje CRC_A, takže je nutné jej přidat předem.

        Vrací přijatá RF data, nebo ``None``, pokud TWN4 vrátí Result=false.
        """
        if not tx:
            raise ValueError("TX nesmí být prázdné.")
        if len(tx) > 0xFF:
            raise ValueError("TX může mít maximálně 255 bajtů.")
        if not 1 <= max_rx_bytes <= 0xFF:
            raise ValueError("max_rx_bytes musí být 1 až 255.")
        if not 0 <= timeout_ms <= 0xFFFF:
            raise ValueError("timeout_ms musí být 0 až 65535.")
        ...
        if not payload:
            raise SerialCommunicationError(
                "ISO14443_3_TDX nevrátil hodnotu Result."
            )
        ...
        if result != 1:
            raise SerialCommunicationError(
                f"ISO14443_3_TDX vrátil neplatný Bool: {result}"
            )

        if len(payload) < 2:
            raise SerialCommunicationError(
                "ISO14443_3_TDX nevrátil délku RX pole."
            )

        rx_length = payload[1]
        rx_start = 2
        rx_end = rx_start + rx_length
        rx = payload[rx_start:rx_end]

        if len(rx) != rx_length:
            raise SerialCommunicationError(
                f"ISO14443_3_TDX deklaroval {rx_length} RX bajtů, "
                f"ale přišlo pouze {len(rx)}."
            )

        if len(payload) != rx_end:
            raise SerialCommunicationError(
                "ISO14443_3_TDX vrátil neočekávaná data za RX pole: "
                f"{payload[rx_end:].hex().upper()}"
            )
